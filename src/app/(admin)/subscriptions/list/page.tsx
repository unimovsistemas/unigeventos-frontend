...
export default function EventSubscriptionList() {
  ...
  const loadOptions = async (inputValue: string) => {
    ...
    try {
      ...
      return response.content.map((event: EventDataResponse) => ({
        value: event.id,
        label: `${event.name} - ${new Date(event.startDatetime).toLocaleDateString()}`,
      }));
    } catch (error: any) {
  ...
  const handleEventChange = async (selected: any) => {
    ...
    setLoading(true);
    try {
      const res = await getSubscriptionsByEvent(token, selected.value);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto text-white">
      <h1 className="text-3xl font-bold text-orange-400 mb-6">Inscrições por Evento</h1>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {subscriptions.map((sub) => (
          ...
          >
            <h2 className="text-xl font-semibold text-orange-300">{sub.personName}</h2>
            ...
            <p className="text-sm mt-1">
              ...
              <span
                className={`font-semibold ${
                  sub.status === "CONFIRMED" ? "text-green-500" : "text-yellow-500"
                }`}
              >
            ...
            <p className="text-sm text-neutral-300">
              <strong>Transporte:</strong> {TransportationLabels[sub.transportationType] || sub.transportationType}
            </p>
            {sub.ministries && (
              <p className="text-sm text-neutral-300">
